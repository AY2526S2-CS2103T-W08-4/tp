...
2. **Membership ID Limit Change:** We plan to remove the current hard cap on membership IDs (currently at 9999). Membership IDs will continue to be unique and auto-generated, and never reuse old IDs from deleted members. This change will make the system more robust against extreme add-delete usage patterns that could otherwise exhaust the available IDs over time.
